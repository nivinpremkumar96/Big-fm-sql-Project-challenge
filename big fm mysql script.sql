-- created database for the data 
CREATE DATABASE fm_station;
-- create a tables into the database 
...
select  showname, count(*) as total_awards_won
from awards a
join shows s
...
SELECT hostname ,year(joindate) as joindate 
from hosts
...
SELECT hostname,sum(showcount) as total_showcount  
from hosts 
group by hostname ;
...
select 	StationName,year(LaunchDate) as launchyear,location 
from stations 
